...
import static com.itsybitso.entity.Tick.TICK_DOWN;
import static com.itsybitso.entity.Tick.TICK_UP;
import static com.itsybitso.entity.Tick.TICK_ZERO;
 ...
 * @author laszlo
 */
public class ConfigurableTrader extends BusinessLogic {

  private static final Logger LOGGER = LoggerFactory.getLogger(ConfigurableTrader.class);
  ...
  private static final Comparator<Trade> reverseTid = Comparator.comparing(Trade::getTid);
  private Trade[] wipTrades;
  ...
  private Long mostRecentTradeTidFromLastBatch = 0L;

  public ConfigurableTrader(WindowConfig windowConfig) {
    downN = windowConfig.getDowns();
    upM = windowConfig.getUps();
    id = windowConfig.getId();
  }

  public void getAndEnhanceRecentTrades() throws Exception {
    mergePretendTrades();
    updateTradesWithTicks();
  }

  public void updateDisplay() throws Exception {
    AppMonitors.getDisplayData(id).setRecentTrades(Arrays.asList(wipTrades));
    BigDecimal latestPrice = wipTrades[0].getPrice().divide(TradesPoller.getUsdMxn(), RoundingMode.DOWN);
    AppMonitors.getDisplayData(id).setLatestPrice(latestPrice);
    AppMonitors.getDisplayData(id).setAccountValue((AppMonitors.getDisplayData(id).getCoinBalance()
        .multiply(latestPrice))
        .add(AppMonitors.getDisplayData(id).getCurrencyBalance())
        .setScale(2, RoundingMode.DOWN));
    if (startingAccountValue == null) {
      startingAccountValue = AppMonitors.getDisplayData(id).getAccountValue();
    }
    AppMonitors.getDisplayData(id).setStartingAccountValue(startingAccountValue);
    AppMonitors.getDisplayData(id).setProfit(AppMonitors.getDisplayData(id).getAccountValue().subtract(startingAccountValue).setScale(2, RoundingMode.DOWN));
  }

  /**
   * multiply trade ID-s by 10 so we can insert another pretend-trade in between,
   * then merge with pretend-trades; public for testing
   *
   */
  public void mergePretendTrades() {

    adjustTradeIds();
    Stream<Trade> pretendStream = pretendTrades.stream();
    Stream<Trade> tradesStream = Arrays.stream(wipTrades);
  ...
  public void updateTradesWithTicks() throws Exception {
    // reverse the order of tid-s so that we can go from oldest to newest for tick info gathering
    wipTrades = Arrays.stream(wipTrades).sorted(reverseTid).toArray(Trade[]::new);
    // set tick for each trade in order of trade id
    setTickAndStatus();
    // reverse list for display..
    wipTrades =  Arrays.stream(wipTrades).sorted(reverseTid.reversed()).toArray(Trade[]::new);
  }
  ...
  public void makeTradeDecision() {
    List<Trade> recentTrades = Arrays.asList(wipTrades);
    int i = 0;
    while (recentTrades.get(i) != null && recentTrades.get(i).getTid().compareTo(mostRecentTradeTidFromLastBatch) > 0) {
      ...
      if (nextNewTrade.getNthStatus().equals(TICK_DOWN.toString() + downN) &&
          nextNewTrade.getTick().equals(TICK_DOWN)) {
        Future<Trade> f = TradesPoller.getInstance().asyncTrade(this, recentTrades.get(0), MARKER_SIDE_SELL);
        break;
      }
      if (nextNewTrade.getNthStatus().equals(TICK_UP.toString() + upM) &&
          nextNewTrade.getTick().equals(TICK_UP)) {
        Future<Trade> f = TradesPoller.getInstance().asyncTrade(this, recentTrades.get(0), MARKER_SIDE_BUY);
        break;
      }
      i++;
    }
   ...
   */
  public Trade trade(Trade mostRecentTrade, String typeOfTrade) throws Exception {
    Trade pretendTrade;
    switch (typeOfTrade) {
      case MARKER_SIDE_BUY:
        pretendTrade = new Trade(BOOK, mostRecentTrade.getCreatedAt(), new BigDecimal(PropertiesUtil.getProperty("buy.amt")).setScale(4, RoundingMode.DOWN), typeOfTrade,
            mostRecentTrade.getPrice(), mostRecentTrade.getTid() + 5);
        break;
      default:
        pretendTrade = new Trade(BOOK, mostRecentTrade.getCreatedAt(), new BigDecimal(PropertiesUtil.getProperty("sell.amt")).setScale(4, RoundingMode.DOWN), typeOfTrade,
            mostRecentTrade.getPrice(), mostRecentTrade.getTid() + 5);
    }
  ...
  private void updateBalances(Trade pretendTrade) throws Exception {
    DisplayData displayData = AppMonitors.getDisplayData(id);
    switch (pretendTrade.getMakerSide()) {
      case MARKER_SIDE_BUY:
        displayData.setCurrencyBalance(displayData.getCurrencyBalance().subtract(
            pretendTrade.getAmount().multiply(
                pretendTrade.getPrice().divide(TradesPoller.getUsdMxn(), RoundingMode.DOWN)
            ).multiply(new BigDecimal(1.01))
        ).setScale(2, RoundingMode.DOWN));
        displayData.setCoinBalance(displayData.getCoinBalance().add(pretendTrade.getAmount()).setScale(2, RoundingMode.DOWN));
        break;
      case MARKER_SIDE_SELL:
        displayData.setCurrencyBalance(displayData.getCurrencyBalance().add(
            pretendTrade.getAmount().multiply(
                pretendTrade.getPrice().divide(TradesPoller.getUsdMxn(), RoundingMode.DOWN)
            ).multiply(new BigDecimal(0.99))
        ).setScale(2, RoundingMode.DOWN));
        displayData.setCoinBalance(displayData.getCoinBalance().subtract(pretendTrade.getAmount()).setScale(2, RoundingMode.DOWN));
        break;
      default:
        LOGGER.info("!!! WRONG MARKER SIDE: " + pretendTrade.getMakerSide());
    }
  }
   ...
   *
   */
  private void adjustTradeIds() {
//    LOGGER.info("******************************** " + id + " " + wipTrades[0].getTid());
    Arrays.stream(wipTrades).forEach(trade -> trade.setTid(trade.getTid() * 10));
//    for (Trade trade : wipTrades) {
//      trade.setTid(trade.getTid() * 10);
//    }
  }

  public void setPretendTrades(Trade[] trades) {
    pretendTrades = new ArrayList<>(Arrays.asList(trades));
  }

  // for testing
  public Trade[] getPretendTrades() {
  ...
  public void setDownN(String downN) {
    this.downN = downN;
  }
  ...
  public void setMostRecentTradeTidFromLastBatch(Long mostRecentTradeTidFromLastBatch) {
    this.mostRecentTradeTidFromLastBatch = mostRecentTradeTidFromLastBatch;
  }

  public void setWipTrades(Trade[] wipTrades) {
    this.wipTrades = wipTrades;
  }
}
